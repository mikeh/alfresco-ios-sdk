...
- (void)testRetrieveRootFolder
{
    if (self.setUpSuccess)
    {
        ...
        XCTAssertNotNil([[[sessionRootFolder properties] objectForKey:@"cmis:path"] value], @"Expected the path to the root folder not to be nil");
        XCTAssertNotNil([[[sessionRootFolder properties] objectForKey:@"cmis:objectId"] value], @"Expected the objectId not to be nil");
        XCTAssertNotNil([[[sessionRootFolder properties] objectForKey:@"cmis:objectTypeId"] value], @"Expected the objectTypeId not to be nil");
        XCTAssertTrue([[[[sessionRootFolder properties] objectForKey:@"cmis:objectTypeId"] value] isEqualToString:@"cmis:folder"], @"Expected the objectTypeID to be a cmis folder type");
...
- (void)testRetrieveSessionRepositoryInformation
{
    if (self.setUpSuccess)
    {
        ...
        else
        {
            ...
            XCTAssertTrue([sessionRepositoryInfo.edition isEqualToString:@"Alfresco in the Cloud"], @"Expected the edition to be cloud edition, but instead got %@", sessionRepositoryInfo.edition);
...
- (void)testOAuthSerialization
{
    if (self.setUpSuccess)
    {
        ...
        XCTAssertEquals(3600, [archivedOAuthData.expiresIn intValue], @"Expires in should be 3600, but instead it is %d",[archivedOAuthData.expiresIn intValue]);
     }
    else
    {
        XCTFail(@"Could not run test case: %@", NSStringFromSelector(_cmd));
                                                        ...
                                                        request = [weakDfService retrieveContentOfDocument:[array objectAtIndex:0] completionBlock:^(AlfrescoContentFile *contentFile, NSError *error)
                                                                   {
                                                                       if (nil == contentFile)
                                                                       {
                                                                           ...
                                                                           XCTAssertEquals([error code], kAlfrescoErrorCodeNetworkRequestCancelled, @"The expected error code is %d, but instead we get %d", kAlfrescoErrorCodeNetworkRequestCancelled, [error code]);
                                                                       }
                                                                       else
                                                                       {
                                                                           self.lastTestSuccessful = NO;
