        ...
        [self.siteService retrieveSiteWithShortName:self.testSiteName completionBlock:^(AlfrescoSite *remoteSite, NSError *error) {
            ...
            else
            {
                ...
                [self.siteService addFavoriteSite:remoteSite completionBlock:^(AlfrescoSite *favSite, NSError *favError) {
                    ...
                    {
                        XCTAssertTrue([favSite.identifier isEqualToString:weakSelf.testSiteName], @"The favorite site should be %@ - but received %@", weakSelf.testSiteName, favSite.identifier);
                        XCTAssertTrue(favSite.isFavorite, @"site %@ should be set to isFavorite", favSite.identifier);
                        XCTAssertTrue(favSite.isPendingMember == remoteSite.isPendingMember, @"pending state should be the same for favourited site");
                        XCTAssertTrue(favSite.isMember == remoteSite.isMember, @"member state should be the same for favourited site");
                        [weakSelf.siteService removeFavoriteSite:favSite completionBlock:^(AlfrescoSite *unFavSite, NSError *unFavError){
                            ...
                            {
                                XCTAssertTrue([unFavSite.identifier isEqualToString:weakSelf.testSiteName], @"The favorite site should be %@ - but instead we got %@", weakSelf.testSiteName, favSite.identifier);
                                XCTAssertFalse(unFavSite.isFavorite, @"site %@ should no longer be a favorite",unFavSite.identifier);
                                XCTAssertTrue(unFavSite.isPendingMember == remoteSite.isPendingMember, @"pending state should be the same for unfavourited site");
                                XCTAssertTrue(unFavSite.isMember == remoteSite.isMember, @"member state should be the same for unfavourited site");
