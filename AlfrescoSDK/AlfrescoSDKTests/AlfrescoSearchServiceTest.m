        ...
        if (!self.isCloud)
        {
            ...
            [self.searchService searchWithStatement:searchStatement language:AlfrescoSearchLanguageCMIS listingContext:listingContext completionBlock:^(AlfrescoPagingResult *pagingResult, NSError *error) {
                ...
                else
                {
                    ...
                    {
                        XCTAssertTrue([pagingResult.objects count] == 25, @"Expected the objects array to contain 25 objects, instead we got back %i", [pagingResult.objects count]);
                    }
                    else
                    {
                        XCTAssertTrue([pagingResult.objects count] <= 25, @"Expected back less than or upto 25 items in the search result, instead got back %i", [pagingResult.objects count]);
                    }
